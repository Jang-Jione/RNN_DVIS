...
from mask2former_video.modeling.transformer_decoder.video_mask2former_transformer_decoder import SelfAttentionLayer,\
    CrossAttentionLayer, ScalingCrossAttentionLayer, FFNLayer, MLP, _get_activation_fn
...
class ReferringTracker(torch.nn.Module):
    def __init__(
        ...
        self.dim_reducer = nn.Linear(25600, 2560)

        self.rnn_block = nn.RNN(
            input_size=2560,
            hidden_size=2560, # hyper-parameter
            num_layers=1,
            batch_first=True
        )

        self.dim_expander = nn.Linear(2560, 25600)

        self.final_cross_attention_layer = ScalingCrossAttentionLayer(
            d_model=hidden_channel,
            nhead=num_head,
            dropout=0.0,
            normalize_before=False,
    ...
    def forward(self, frame_embeds, mask_features, resume=False, return_indices=False):
        ...
        # rnn block
        frames, layers, query_len, batch_size, embedding_len = outputs.shape
        temp_data = outputs[:, -1, :, :, :].reshape(batch_size, frames, query_len * embedding_len)
        reduced_data = self.dim_reducer(temp_data)
        input_data = reduced_data.reshape(batch_size, frames, 2560)  # (b, t, cq)

        rnn_outputs, _ = self.rnn_block(input_data)
        rnn_outputs = rnn_outputs.permute(1, 0, 2)  # (t, b, 1000)

        expanded_outputs = self.dim_expander(rnn_outputs)
        expanded_outputs = expanded_outputs.reshape(frames, batch_size, query_len, embedding_len)  # (t, b, q, c)
        rnn_outputs = expanded_outputs.permute(0, 3, 2, 1)  # (t, c, q, b)

        outputs = outputs[:, -1]  # (t, q, b, c)

        # Final cross attention
        rnn_outputs = rnn_outputs.permute(3, 2, 0, 1)  # (b, q, t, c)
        rnn_outputs = rnn_outputs.reshape(batch_size * frames, query_len, embedding_len)

        outputs = outputs.permute(2, 0, 1, 3)  # (b, t, q, c)
        outputs = outputs.reshape(batch_size * frames, query_len, embedding_len)

        final_outputs = self.final_cross_attention_layer(
            rnn_outputs, outputs,
            memory_mask=None,
            memory_key_padding_mask=None,
            pos=None, query_pos=None,
        ...
        final_outputs = final_outputs.reshape(batch_size, frames, query_len, embedding_len)  # (b, t, q, c)
        final_outputs = final_outputs.permute(1, 2, 0, 3)  # (t, q, b, c)

        final_outputs = final_outputs.unsqueeze(1)  # (t, 1, q, b, c)
        final_outputs = torch.cat([temp_outputs, final_outputs], dim=1)  # (t, l+1, q, b, c)

        # Final prediction heads
        final_class, final_masks = self.prediction(final_outputs, mask_features)
        final_outputs = self.decoder_norm(final_outputs)

        final_out = {
            'pred_logits': final_class[-1].transpose(1, 2),  # (b, t, q, c)
            'pred_masks': final_masks[-1],  # (b, q, t, h, w)
            'aux_outputs': self._set_aux_loss(final_class, final_masks),
            'pred_embds': final_outputs[:, -1].permute(2, 3, 0, 1)  # (t, b, c, q)
        }

        if return_indices:
            return final_out, ret_indices
        else:
            return final_out
