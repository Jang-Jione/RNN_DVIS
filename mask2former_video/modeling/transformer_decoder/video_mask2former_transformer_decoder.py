...
class ScalingCrossAttentionLayer(nn.Module):

    def __init__(self, d_model, nhead, dropout=0.0,
                 activation="relu", normalize_before=False, scaling_factor=1.0):
        super().__init__()
        self.multihead_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
    ...
    def forward_post(self, tgt, memory,
                     ...
                     query_pos: Optional[Tensor] = None,
                     scaling_factor=1.0):
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos)*scaling_factor,
                                   key=self.with_pos_embed(memory, pos),
                                   value=memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
        tgt = tgt + self.dropout(tgt2)
    ...
    def forward_pre(self, tgt, memory,
                    ...
                    scaling_factor=1.0):
        tgt2 = self.norm(tgt)
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt2, query_pos)*scaling_factor,
                                   key=self.with_pos_embed(memory, pos),
                                   value=memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
        tgt = tgt + self.dropout(tgt2)
